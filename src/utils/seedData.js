...
const seedData = async () => {
  try {
    ...
    for (const envelope of createdEnvelopes) {
      const usageCount = donations.filter(
        (d) =>
          d.envelope.toString() === envelope._id.toString() &&
          d.status === "completed"
      ).length;
...
const createDefaultAdmin = async () => {
  try {
    ...
    if (!existingAdmin) {
      // Le mot de passe sera haché automatiquement par le middleware du modèle
...
module.exports = { seedData, createDefaultAdmin };
