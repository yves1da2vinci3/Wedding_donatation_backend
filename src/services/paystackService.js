    ...
    async initializeMobileMoneyTransaction(amount, email, provider, phone, currency = 'XOF', callback_url = null) {
        try {
            ...
            console.log('Initializing mobile money payment:', {
                ...
                amount,
                currency,
                phone: phone.substring(0, 3) + '****' + phone.substring(phone.length - 2) // Log masked phone
            });
            ...
            if (responseData.status) {
                ...
                if (provider.toLowerCase() === 'orange' && responseData.data?.status === 'send_otp') {
            ...
            return {
                status: false,
                message: error.response?.data?.message || 'Failed to initialize mobile money payment',
